...
ツイート内容に対して正規表現でのマッチングを行い、クロージャまたは文字列から結果を得ます。<br />
...
- (array) __*$pairs*__<br />**「正規表現 => クロージャ」** 、または  **「正規表現 => 文字列」** の形の配列。<br />クロージャの第1引数は **ステータスオブジェクト** 、第2引数は **マッチ結果の配列** になります。<br />文字列には `${0}` `${1}` `${2}` ... という置換用フォーマットを使うことが出来ます。<br />以下に例を示します。
...
なお、この先頭のものは、もし「～さん」という名前呼びをつけなくてもいいのであれば、クロージャを使う代わりに文字列を使う方式でも書くことが出来ます。クロージャを生成しなくてもいい単純な置換には、こちらを適用するといいでしょう。
